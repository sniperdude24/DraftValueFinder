...
export function marketComparison(players, assessments = null) {
  ...
  const meaningful = rows
    .filter(r => r.ai_vs_adp != null || r.ai_vs_expert != null)
    .map(r => ({ ...r, max_disagreement: Math.max(Math.abs(r.ai_vs_adp ?? 0), Math.abs(r.ai_vs_expert ?? 0), Math.abs(r.expert_vs_adp ?? 0)) }))
